...
export default (Avatar = props => {
  ...
  const { source, name, username, onSelectUser, size } = props;
  let useInitials = false;
  if (source === 'https://church-groups.cratebind.com/avatars/original/missing.png') {
    useInitials = true;
  }
  if (source === 'https://church-groups.cratebind.com/logos/original/missing.png') {
    useInitials = true;
  }
  ...
  const avatar = useInitials ? (
    <StyledTouchableAvatar onPress={onSelectUser}>
      <StyledEmptyProfile size={size}>
        <StyledInitials>{`${name}${username}`}</StyledInitials>
      </StyledEmptyProfile>
